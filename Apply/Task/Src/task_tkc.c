...
void TuikongData_Send(void) //下行数据发送函数，即向推控舱数据定时发送 0.2秒发送一次到推控舱
{
	...
	Tuikong_SendData[27] = Downlink_Data.Depthometer_Data*10 / 256; //深度计数据
...
//	Drv_Uart_Transmit(&tTKC_Uart, (uint8_t *)"@TT", 3);
//	Drv_Uart_Transmit(&tTKC_Uart, Tuikong_SendData, 32);
