...
void ShumeiData_Send(void) //上行数据发送函数，即向树莓派数据定时发送 0.2秒发送一次到树莓派
{
	...
	Shumei_SendData[4] = Uplink_Data.Depthometer_Data*10 / 256; //深度计数据
	Shumei_SendData[5] = (uint8_t)(Uplink_Data.Depthometer_Data*10) % 256;
	Shumei_SendData[6] = Uplink_Data.Altimeter_Data*100 / 256; //高度计数据
	Shumei_SendData[7] = (uint8_t)(Uplink_Data.Altimeter_Data*100) % 256;
